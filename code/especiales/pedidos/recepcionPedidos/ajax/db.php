...
//echo 'here';
	...
	function getProviderInvoices( $link, $provider_id ){
		...
		$sql="SELECT
			...
			pp.id_proveedor,
			1 AS is_in_order/*implementacion para que no se pinte de rojo*/ 
				FROM(
					SELECT 
						ocd.id_oc_detalle,
	 				 	ocd.id_producto,
		...
		while( $row = $exc->fetch_row() ){
			...
			$resp .= build_row( $row, $counter, $provider_id, $link );
		}
		$resp .= '<input type="hidden" id="filas_totales" value="' . $counter . '">';
			//echo $resp;
		return $resp;
		//return build_row( $exc );
	}
	function build_row( $r, $c, $provider_id, $link ){
		$resp = "";
		$background = "";
		if( $r[12] <= 0 ){
		...
		//nombre del producto
			$resp .= '<td id="2_'.$c.'" class="product_description" style="padding:10px;" width="15%">'.$r[2].'</td>';
	...
	function getReceptionDetail( $reception_id, $provider_id, $link ){
		$resp = "";
		$sql="SELECT
				ax1.id_recepcion_bodega_detalle,
				ax1.id_producto,
				ax1.nombre,
				ax1.cantidad,
				ax1.recibido,
				ax1.cajas_recibidas,
				ax1.piezas_sueltas_recibidas,
				ax1.piezas_por_caja,
				IF(pp.precio_pieza IS NULL, 0, pp.precio_pieza) as precio_pieza,
				ax1.ubicacion_almacen,
				ax1.id_proveedor_producto,
				pp.id_proveedor,
				(SELECT 
			...
			ORDER BY ax1.numero_partida ASC";
	...
	function finishReception( $id, $link ){
		$sql = "UPDATE ec_recepcion_bodega
					SET serie = IF( id_status_validacion IN( 2, 3 ), serie,  CONCAT( serie, '_', id_recepcion_bodega ) ), 
