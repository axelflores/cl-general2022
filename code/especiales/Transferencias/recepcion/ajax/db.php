<?php
	...
	function validateBarcode( $barcode, $transfers, $user, $excedent_permission = null, $pieces_quantity = null, $link ){
	//verifica si el código de barras existe
		...
		if( $stm1->num_rows <= 0 ){
			return 'exception|<br/><h3 class="inform_error">El código de barras no esta registrado en ningún producto.</h3>' 
				. '<div class="row"><div class="col-2"></div><div class="col-8">'
				. '<button class="btn btn-danger form-control" onclick="document.getElementById(\'barcode_seeker\').value=\'\';close_emergent();">Aceptar</button></div><br/><br/>';
		}
	...
	//verifica si el producto existe en la transferencia
		if( $stm2->num_rows <= 0 ){
			...
			if( $stm3->num_rows <= 0){
				...
				$resp = 'exception|<br/><h3 class="inform_error">El producto no corresponde a la(s) Transferencia(s)<br />Si se va a quedar la mercancía, pida la autorización del encargado : </h3>'; 
				...
				$resp = 'exception|<br/><h3 class="inform_error">El modelo del producto no corresponde a la(s) Transferencia(s)<br />Si se va a quedar la mercancía, pida la autorización del encargado : </h3>'; 
				$resp .= '<div class="row"><div class="col-2"></div><div class="col-8">';
				$resp .= '<input type="password" id="manager_password" class="form-control emergent_manager_password"><br />';
				$resp .= '<button class="btn btn-danger form-control" onclick="save_new_reception_detail( ';
					$resp .= " {$inform['product_id']}, {$inform['product_provider_id']}, {$inform['box']}, {$inform['pack']}, {$inform['piece']} ";
	...
	//verifica transferencias pendientes de validación	
		...
		$stm = $link->query( $sql ) or die( "|Error al consultar transferencias pendientes de validar : " . $link->error );
		...
		switch ( $type ) {
			case 1:
				...
				$sql = "SELECT 
							...
							( tp.total_piezas_validacion - tp.total_piezas_recibidas ) AS difference,
							tp.id_producto_or AS product_id,
...
//obtener detalle de la recepción
	...
	//inserta el registro de resolución de transferencias
		...
		$stm = $link->query( $sql ) or die( "Error al insertar el registro de resolcuión de transferencias : {$link->error}" );
